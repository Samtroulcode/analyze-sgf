...
// 'aa' => 'A19'
// 'bd' => 'B16'
function iaToJ19(value, boardYSize) {
  const v = value.toUpperCase();
  return v[0] >= 'I'
    ? nextChar(v[0]) + (65 + boardYSize - v.charCodeAt(1)).toString()
    : v[0] + (65 + boardYSize - v.charCodeAt(1)).toString();
}
...
// ';W[po]' => 'Q5'
function seqToPV(seq, boardYSize) {
  const pl = seq[0] === '(' ? seq[2] : seq[0];
  const pv = seq
    .split(';')
    .filter((move) => move.search(/[BW]\[[^\]]/) === 0)
    .map((move) => iaToJ19(move.substring(2, 4), boardYSize))
    .join(' ');
