...
Compact classification uses the `ogs` profile by default, following
Online-Go.com's score-loss categories: `Excellent`, `Great`, `Good`,
`Inaccuracy`, `Mistake`, and `Blunder`. The optional `hybrid` profile combines
score loss and winrate impact, and can additionally show `Best` and
`Missed opportunity`. In Sabaki, best/excellent moves use the blue interesting
marker, great/good moves use the green good marker, doubtful moves use the
doubtful marker, and real mistakes use bad/hotspot markers.
